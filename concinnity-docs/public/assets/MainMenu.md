...
- `dim`: An array of 4 floats. RGBA fill drawn across the whole window behind the items. An alpha of 0 draws no backdrop. A fully opaque alpha (1.0) hides the scene completely, which lets the renderer skip the entire world render while the menu is open: the frame then costs only the menu overlay. Defaults to `[0.0, 0.0, 0.0, 0.55]`.
