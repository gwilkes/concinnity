...
impl<'a> PipelineContext<'a> {
    ...
    pub fn query_mut<C: ComponentSlot>(&mut self) -> std::slice::IterMut<'_, C> {
        C::slot_mut(self.components).iter_mut()
    }
    ...
    pub fn query_slice_mut<C: ComponentSlot>(&mut self) -> &mut [C] {
        C::slot_mut(self.components).as_mut_slice()
    }

    // Remove and return all components of type C. O(1): swaps in an empty
    // vec for the typed slot.
    pub fn drain<C: ComponentSlot>(&mut self) -> Vec<C> {
        std::mem::take(C::slot_mut(self.components))
    }

    // Push a runtime-produced component into the matching typed slot.
    // Preferred over reaching into `self.components` directly.
    #[allow(dead_code)]
    pub fn push<C: ComponentSlot>(&mut self, c: C) {
        C::slot_mut(self.components).push(c);
    }
...
macro_rules! define_components {
    ( $( $variant:ident => $ty:path, $disc:expr_2021 ),+ $(,)? ) => {
        ...
        // Per-type runtime storage. One `Vec<T>` per registered component
        // type, eliminating the `sizeof(largest_variant)` overhead that a
        // single `Vec<ComponentAsset>` paid. Field names are the variant
        // names from the registry, accessed through the `ComponentSlot`
        // trait below; callers never name the fields directly.
        ...
        impl ComponentStorage {
            // Dispatch a `ComponentAsset` variant into the matching typed vec.
            pub fn push(&mut self, asset: ComponentAsset) {
                match asset {
                    $( ComponentAsset::$variant(c) => self.$variant.push(c), )+
                }
            }
        ...
        // `C` to its dedicated slot inside `ComponentStorage` at compile time.
        pub trait ComponentSlot: Component {
            fn slot(s: &ComponentStorage) -> &Vec<Self>;
            fn slot_mut(s: &mut ComponentStorage) -> &mut Vec<Self>;
        }

        $(
            impl ComponentSlot for $ty {
                fn slot(s: &ComponentStorage) -> &Vec<Self> { &s.$variant }
                fn slot_mut(s: &mut ComponentStorage) -> &mut Vec<Self> { &mut s.$variant }
            }
        )+
    };
}
