...
impl MtlContext {
    ...
    // scene snapshot, and back-to-front sorting. Each draw binds the snapshot
    // (refraction source) at texture(0), the resolved main depth at
    // texture(1), and the IBL prefilter cube at texture(2). Sampling the
    // snapshot rather than `hdr_resolve` is what lets water render with SSR
    // off.
    pub(in crate::metal) fn collect_water_transparent_draws(
        ...
        let pipeline = match &self.water_pipeline {
            Some(p) => p,
            None => return,
        };
        ...
        for surface in &self.water_surfaces {
            ...
            out.push(TransparentDraw {
                ...
                fragment_textures: vec![
                    ...
                    (2, self.env_map.prefilter.clone()),
                ],
                fragment_samplers: vec![
                    (0, self.post_sampler.clone()),
                    (1, self.cube_sampler.clone()),
                ],
                sort_distance,
            });
        }
...
pub(super) fn build_water_pipeline(
    ...
    let msl = shader_source(hot_reload, "water.metal");
    let options = objc2_metal::MTLCompileOptions::new();
    let library = device
        .newLibraryWithSource_options_error(&NSString::from_str(msl.as_ref()), Some(&options))
        .map_err(|e| format!("water shader compile error: {:?}", e))?;

    let vert_fn = library
        .newFunctionWithName(&ns_str("water_vertex"))
        .ok_or("water_vertex not found")?;
    let frag_fn = library
        .newFunctionWithName(&ns_str("water_fragment"))
        .ok_or("water_fragment not found")?;
